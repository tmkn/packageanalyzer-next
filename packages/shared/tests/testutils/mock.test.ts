...
import { DependencyUtilities } from "@lib/shared";
import {
    createMockPackage,
    MockProvider,
    type IMockPackageJson
} from "../../src/testutils/mocks.js";
...
describe(`Mock Tests`, () => {
    ...
    test(`MockProvider returns data`, async () => {
        ...
        const p = await provider.getPackageVersionMetadata(`mockpackage`, `1.2.3`);
    ...
    test(`MockProvider throws on missing data`, async () => {
        ...
        await expect(
            provider.getPackageVersionMetadata(`doesnt_exist`, `1.2.3`)
